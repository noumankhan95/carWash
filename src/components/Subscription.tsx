...
function Subscription({ settheStep }: AddStaffMemberChildrenProps) {
  ...
  return (
    <div>
        ...
        <div className="flex">
          ...
          >
            <EditSubscriptionModal
              // addSubscription={(packages) => {
              //   setsubscriptions((p) =>
              //     p?.some((i) => i.name == services)
              //       ? p
              //       : [...(p || []), { name: services, packages }],
              //   );
              // }}
              name={services}
            />
      ...
      <div className="rounded-sm border border-stroke space-y-5 bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <select
            ...
            <tbody>
              {Subscriptions?.map((s) => (
                <tr>
                      ...
                      <>
                        {p.Once.isrunning && (
                          <p className="text-black dark:text-white">
                            {p.Once.duration} per week : {p.Once.discount}%
                          </p>
                        )}
                        {p.Twice.isrunning && (
                          <p className="text-black dark:text-white">
                            {p.Twice.duration} per week : {p.Twice.discount}%
                          </p>
                        )}
                        {p.Thrice.isrunning && (
                          <p className="text-black dark:text-white">
                            {p.Thrice.duration} per week : {p.Thrice.discount}%
                          </p>
                        )}
                      </>
                    ))}
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium text-success"></p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
