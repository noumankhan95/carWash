...
const Services = () => {
  ...
  const [services, setservices] = useState<ServiceAdditionItem[]>([]);
  const { permissions } = useUserAuth();
  // const { setCategoryItems } = useCategoryStore();
  ...
  const getservices = useCallback(async () => {
    try {
      ...
      const servarray: ServiceAdditionItem[] = [];
      if (!serv.empty) {
        serv.forEach((ser) =>
          servarray.push({
            ...(ser.data() as ServiceAdditionItem),
            id: ser.id,
          }),
        );
      }
      ...
      toast.error('Couldnt Fetch services');
    } finally {
      setisloading(false);
      setreload(false);
    }
              ...
              <li
                ...
                onClick={async () => {
                  try {
                    ...
                    await deleteDoc(doc(db, 'services', todelete!));
                    settodelete('');
                    setshowAlert(false);
                    setreload(true);
