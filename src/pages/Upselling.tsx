...
const Upselling = () => {
  ...
  const [upselling, setupselling] = useState<upselling[]>([]);
  ...
  const getupselling = useCallback(async () => {
    try {
      setisloading(true);
      const cats = await getDocs(collection(db, 'upsellings'));
      const catarray: upselling[] = [];
      if (!cats.empty) {
        cats.forEach((cat) =>
          catarray.push({
            ...(cat.data() as upselling),
            id: cat.id,
          }),
        );
      }
      setupselling(catarray);
    } catch (e) {
              ...
              <li
                ...
                onClick={async () => {
                  try {
                    ...
                    await deleteDoc(doc(db, 'upsellings', todelete!));
                    settodelete('');
                    setshowAlert(false);
                    setreload(true);
